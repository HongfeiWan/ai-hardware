...
partition. The current app image is about 0xe5020 bytes, leaving about 70% free
...
command. From inside the bundle directory:
...
`fixture://status`, reads `fixture://net-map` and, when ADC support is enabled,
performs one raw ADC sample. It does not toggle MUX lines, reset the DUT or
enable the load switch.

After the fixture wiring is confirmed, add `--exercise-net-adc-tool` to also
call `fixture.read_net_adc_raw` on the first configured net-map entry. That
action changes the MUX selection.
...
curl -X POST http://192.168.4.1/mcp \
  ...
  -H "MCP-Protocol-Version: 2024-11-05" \
  -H "MCP-Session-Id: <session-id>" \
  -d '{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"fixture.read_net_adc_raw","arguments":{"net":"TP0","samples":8,"settle_ms":10}}}'
```
...
curl -X POST http://192.168.4.1/mcp \
  ...
  -H "MCP-Protocol-Version: 2024-11-05" \
  -H "MCP-Session-Id: <session-id>" \
  -d '{"jsonrpc":"2.0","id":6,"method":"tools/call","params":{"name":"fixture.reset_dut","arguments":{"pulse_ms":100}}}'
```
...
curl -X POST http://192.168.4.1/mcp \
  ...
  -H "MCP-Protocol-Version: 2024-11-05" \
  -H "MCP-Session-Id: <session-id>" \
  -d '{"jsonrpc":"2.0","id":7,"method":"resources/read","params":{"uri":"fixture://status"}}'
```
...
curl -X POST http://192.168.4.1/mcp \
  ...
  -H "MCP-Protocol-Version: 2024-11-05" \
  -H "MCP-Session-Id: <session-id>" \
  -d '{"jsonrpc":"2.0","id":8,"method":"resources/read","params":{"uri":"fixture://net-map"}}'
```

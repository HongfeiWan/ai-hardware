...
Python 测试站原型在 [bench/ai_hardware_bench](bench/ai_hardware_bench)。这一层不依赖 ESP32 实机，也不强制安装 `mcp`、`fastmcp`、`yaml` 或 `jsonschema`；当前实现用标准库完成板级文件加载、轻量校验、拓扑查询、mock PSU、mock scope、mock fixture、波形特征提取、规则诊断、CSV/Altium CSV/BOM/PnP/KiCad XML/KiCad PCB/KiCad SCH 导入和 MCP-shaped stdio JSON-RPC 入口。
...
stdio MCP server 支持 `tools/list`、`tools/call`、`resources/list`、`resources/templates/list`、`resources/read`、`prompts/list` 和 `prompts/get`。`tools/list` 会为每个工具返回 JSON `inputSchema`，包括必填参数、枚举、数值范围和 `confirm`/`dry_run` 等安全相关参数，便于 MCP Inspector 或客户端生成可调用表单；`tools/call` 入口也会按这些 schema 拒绝缺失、未知或类型/范围不匹配的参数。资源模板包括 `board://context/{board_id}`、`board://topology/{board_id}`、`board://net/{board_id}/{net_name}`、`session://measurements/{session_id}` 和 `session://artifacts/{session_id}/{artifact_id}`；文本 artifact（例如 waveform CSV）会以内联文本返回。当前 prompts 包括 `diagnose_power_rail`、`diagnose_boot_sequence` 和 `plan_next_measurement`，`prompts/list` 同样暴露 `inputSchema`，`prompts/get` 会拒绝未知或类型不匹配的参数，并把已加载的 board/session/topology/measurement 摘要整理成可发给模型的结构化提示。
...
python3 tools/bench.py import-board \
  ...
  --output artifacts/imported-kicad-pcb-board.json \
  --board-id imported_kicad_pcb \
  --name "Imported KiCad PCB Board"

python3 tools/bench.py import-board \
  --format kicad-sch \
  --input path/to/board.kicad_sch \
  --output artifacts/imported-kicad-sch-board.json \
  --board-id imported_kicad_sch \
  --name "Imported KiCad SCH Board"

python3 tools/bench.py import-board \
  --format altium \
  --input path/to/altium-netlist.csv \
  --output artifacts/imported-altium-board.json \
  --board-id imported_altium \
  --name "Imported Altium Board"
...
python3 tools/bench.py import-board \
  --format pnp \
  --input path/to/pick-place.csv \
  --output artifacts/imported-pnp-board.json \
  --board-id imported_pnp \
  --name "Imported Pick Place Board"
```

同样的 CSV/Altium CSV/BOM/PnP/KiCad XML/KiCad PCB/KiCad SCH 内容也可以在本地 `console` 的 Import Board 面板里导入；`KiCad Auto` 会根据粘贴内容识别 XML、`.kicad_pcb` 或 `.kicad_sch`；导入成功后控制台会把当前 board 切换到生成的 `board_context.json`。

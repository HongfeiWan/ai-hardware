...
Python 测试站原型在 [bench/ai_hardware_bench](bench/ai_hardware_bench)。这一层不依赖 ESP32 实机，也不强制安装 `mcp`、`fastmcp`、`yaml` 或 `jsonschema`；当前实现用标准库完成板级文件加载、轻量校验、拓扑查询、mock PSU、mock scope、mock fixture、波形特征提取、规则诊断、CSV/Altium CSV/BOM/PnP/KiCad XML/KiCad PCB 导入和 MCP-shaped stdio JSON-RPC 入口。
...
同样的 CSV/Altium CSV/BOM/PnP/KiCad XML/KiCad PCB 内容也可以在本地 `console` 的 Import Board 面板里导入；导入成功后控制台会把当前 board 切换到生成的 `board_context.json`。
